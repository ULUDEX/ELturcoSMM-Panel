...
export async function GET(request:Request){
 ...
 try{
  ...
  if(!user){const salt=randomHex(16),hash=await passwordHash(randomHex(32),salt),created=Math.floor(Date.now()/1000),result=await env.DB.prepare("INSERT INTO customer_users(name,email,password_hash,password_salt,created_at) VALUES(?,?,?,?,?)").bind(name,email,hash,salt,created).run();user={id:Number(result.meta.last_row_id),name,email};await env.DB.prepare("INSERT OR IGNORE INTO customer_balances(email,balance,created_at,updated_at) VALUES(?,0,?,?)").bind(email,created,created).run()}
