...
export async function POST(request:Request){
 ...
 const userId=Number(result.meta.last_row_id);await env.DB.prepare("INSERT OR IGNORE INTO customer_balances(email,balance,created_at,updated_at) VALUES(?,0,?,?)").bind(email,created,created).run();const session=await createCustomerSession(userId),response=NextResponse.json({ok:true,user:{name,email,balance:0}});response.cookies.set(CUSTOMER_COOKIE,session.token,customerCookieOptions(session.expires));return response
